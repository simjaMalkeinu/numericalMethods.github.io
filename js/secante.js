const headTable = () => {
    ...
    $('#table').html(`<table id="data">
                        <tr>
                            ...
                            <th>
                                <span>x<i>n+1</i> = x<i>n-1</i> -
                                    <div class="fraction">
                                        <span class="fup">x<i>n</i> - x<i>n-1</i></span>
                                        <span class="fdn">f(x<i>n</i>) - f(x<i>n-1</i>)</span>
                                    </div>
                                    f(x<i>n-1</i>)
                                </span>
                            </th>
...
const rowsTable = (sideLeft, sideRigth, data) => {
    ...
    var aux, backX, funBX, xn, funXn, newXn, abs;

    //la iteracion 1, pasamos los datos que ya calculamos 
    backX = sideLeft.x;            //'x' del lado izquierdo inicial
    xn = sideRigth.x;             //'x' del lado derecho inicial

    funBX = sideLeft.y;         //'y' del lado izquierdo inicial
    funXn = sideRigth.y;        //'y' del lado derecho inicial


    do {
        newXn = backX - (((backX - xn) / (funBX - funXn)) * funBX);                 //calculamos el nuevo Xn, o Xn+1
        ...
        // ahora el Xn+1 sera el Xn para la nueva iteracion y el Xn se volvera el backX
        backX = xn;
        xn = newXn;
